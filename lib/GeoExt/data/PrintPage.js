...
GeoExt.data.PrintPage = Ext.extend(Ext.util.Observable, {
    ...
    /** api: property[geodetic]
     *  ``Boolean`` consider geodetic distances for print box drawing.
     */
    geodetic: false,
    ...
    constructor: function(config) {
        ...
        // required to calculate geodesic bounds:
        this.map = config.map instanceof GeoExt.MapPanel ?
    ...
    calculatePageBounds: function(scale, units) {
        ...
        // non geodesic distances:
        var w = size.width / 72 / OpenLayers.INCHES_PER_UNIT[units] * s;
        var h = size.height / 72 / OpenLayers.INCHES_PER_UNIT[units] * s;

        // geodetic distances
        if (this.geodetic) {
            // borrowed & adapted from  map.getGeodesicPixelSize()
            var res = OpenLayers.Util.getResolutionFromScale(s, units);
            var left = center.add(-res / 2, 0);
            var right = center.add(res / 2, 0);
            var bottom = center.add(0, -res / 2);
            var top = center.add(0, res / 2);
            var dest = new OpenLayers.Projection("EPSG:4326");
            var source = this.map.getProjectionObject();
            ...
            var kmToUnitRatio = OpenLayers.INCHES_PER_UNIT["km"] / OpenLayers.INCHES_PER_UNIT[units];
            // OpenLayers.Util.distVincenty always provides the result in km:
            var xRatio = kmToUnitRatio * OpenLayers.Util.distVincenty(left, right) / res;
            var yRatio = kmToUnitRatio * OpenLayers.Util.distVincenty(bottom, top) / res;
            w = w / xRatio;
            h = h / yRatio;
        }

        return new OpenLayers.Bounds(center.lon - w/2, center.lat - h/2,
            center.lon + w/2, center.lat + h/2);
    },
